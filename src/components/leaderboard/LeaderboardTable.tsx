...
async function fetchLeaderboard(): Promise<Player[]> {
    ...
    try {
        ...
        let userIds: string[] = [];
        let usersData: Map<string, any> = new Map();
        ...
        for (const userId of userIds) {
            ...
            const userName = userData.displayName || userData.userName || userData.name || 'Joueur anonyme';
            ...
            try {
                ...
                const gamesSnapshot = await getDocs(gamesQuery);
                    ...
                    try {
                        ...
                        if (isCecilia) {
                            ...
                            levelStatsSnap.docs.forEach(levelDoc => {
                                ...
                                const levelScoreRaw = levelData.score_global ?? levelData.scoreGlobal ?? 
                                ...
                                // Track max level
                                if (levelDoc.id.toLowerCase().includes('niveau') || 
                            ...
                            if (totalScore === 0) {
                                const gameGlobalRaw = gameData.score_global ?? gameData.scoreGlobal ?? gameData.globalScore ?? gameData.score ?? 0;
                            ...
                            const gameGlobalRaw = gameData.score_global ?? gameData.scoreGlobal ?? 
                    ...
                    const isTotal = characterName.toLowerCase().includes('total') || 
                                   characterName.toLowerCase().includes('cécité totale');
                    const mode: 'total' | 'partial' = isTotal ? 'total' : 'partial';
                    
                    // Get completion time
                    const completionTime = gameData.totalTime ?? gameData.completionTime ?? 
                    ...
                    const levelDisplay = isFinished
                        ...
                        : (maxLevel !== 'N/A' 
                            ? maxLevel 
                            : (gameData.currentLevel ? `Niveau ${gameData.currentLevel}` : 'En cours'));
                    
                    // Numeric level for ranking
                    let levelNum = maxLevelNum;
                    if (isFinished) levelNum = 99; // finished games rank highest
                    else if (!levelNum && typeof gameData.currentLevel === 'number') levelNum = gameData.currentLevel;
...
function LeaderboardSection({ 
    ...
    const modeColor = mode === 'total' ? 'bg-purple-100 text-purple-800 border-purple-200' : 'bg-blue-100 text-blue-800 border-blue-200';
    ...
    return (
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
            {/* Header */}
            <div className={`p-6 border-b-2 ${mode === 'total' ? 'border-purple-200 bg-purple-50' : 'border-blue-200 bg-blue-50'}`}>
                <div className="flex items-center justify-center gap-3">
                    <span className="text-3xl">{modeIcon}</span>
                    <h3 className="text-2xl font-bold">{title}</h3>
                </div>
                <p className="text-center text-gray-600 mt-2">{players.length} joueur{players.length > 1 ? 's' : ''}</p>
            </div>

            {/* Top 3 */}
            {rankedPlayers.length > 0 && (
                <div className="p-6 bg-gray-50">
                    <div className={`grid gap-4 ${rankedPlayers.length >= 3 ? 'grid-cols-3' : rankedPlayers.length === 2 ? 'grid-cols-2' : 'grid-cols-1'}`}>
                        {rankedPlayers.slice(0, 3).map((player, idx) => (
                            <div 
                                key={player.id} 
                                className={`text-center p-4 rounded-xl ${
                                    player.rank === 1 ? 'bg-yellow-100 border-2 border-yellow-400 scale-110' :

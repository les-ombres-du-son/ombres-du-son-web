    ...
    >
      ...
      <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
        ...
        <div className="mb-12">
          ...
          <p className="text-2xl md:text-3xl text-white/90 drop-shadow-lg font-light">de vision</p>
        </div>
        ...
        <div className="bg-black/60 backdrop-blur-xl rounded-3xl p-8 md:p-10 border border-white/20 shadow-2xl transition-all duration-300">
          ...
          <p className="text-lg md:text-xl text-white/80 mb-6">{level.description}</p>
          {level.affected && (
            <div className="pt-6 border-t border-white/20">
              <p className="stat-number text-2xl md:text-3xl text-accent">{level.affected}</p>
            </div>
          )}
        </div>
          ...
          {visionLevels.map((_, index) => (
            <div
              ...
              className={`h-1.5 rounded-full transition-all duration-300 ${
                index === currentLevel ? 'bg-primary w-12' : 'bg-white/30 w-8'
              }`}
              aria-hidden="true"
            />
          ))}
